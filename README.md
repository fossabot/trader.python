...
*Generate or retrieve your bitcoin deposit address
...
###History 
*"btchistory" or "usdhistory" - Prints out your entire trading history of BTC transactions or USD, write it to a file, and do a small analysis (fees, BTC/$in, BTC/$out)= 
*"tradehist24" - Download the entire trading history of everyone on Mt Gox for the past 24 hours, and write it to a file. 
*"readtradehist24" - Analyze the above trading history file (High/low/vwap/total/amounts/times)
...
*"Stopbot" = a stop loss bot! Finally, you can set your position price, position amount, and % willing to accept on your position. ie: 5 BTC @ $149 * 98% and the bot will put a market order in IMMEDIATELY (every 2 seconds it checks if the lowest seller is below your target percentage price, and if it is, sends the order immediately)
    *(stopbot is for mtgox only so far it is the only one to support a true market order)
...
encrypt_apikey.py:  	You need to generate encrypted keys beforehand with this command
			*Sometimes the generation does not work (just re-run this script; sometimes takes 2-3 times until it is verified)*
...
###bin\
*-encrypt_api_key - REQUIRED to use this suite, generates an encrypted API key file in keys\ with the site name (see above) encrypted with a password
